...
// Функция для выполнения команды движения
    ...
    if (e.button === 0 && state.currentOrder) {
        ...
        if (clickedEnt && clickedEnt !== state.selectedEntity) {
            ...
            if (actionQueue.shiftPressed) {
                ...
                console.log(`⚡ Очистка очереди и немедленное движение в ${tx},${ty}`);
                actionQueue.clear();
                entity.path = [];
                entity.target = null;
                entity.job = null;
...
function updateInspectPanel(ent) {
    ...
    content.innerHTML = `
        ...
        <p style="color: #81d4fa; font-size: 0.8em;">
            ...
            ${actionQueue.queue.length ? ` | 📋 В очереди: ${actionQueue.queue.length}` : ''}
        </p>
    `;
}
...
function render() {
    ...
    const endCY = Math.ceil((worldViewTop + viewH) / chunk
const canvas = document.getElementById('gameCanvas');
const ctx = canvas.getContext('2d');
...
const state = {
    ...
    map: { width: 100, height: 100, tileSize: 32, tiles: [], fogOfWarEnabled: false },
...
const TILE_TYPES = {
    GRASS: { color: '#4a7c44', name: 'Grass', moveCost: 1 },
    WATER: { color: '#1976d2', name: 'Water', solid: true },
    TREE: { color: '#1b5e20', name: 'Tree', solid: true, harvestable: 'wood' }
};
...
function initMap() {
    ...
    for (let y = 0; y < state.map.height; y++) {
        ...
        for (let x = 0; x < state.map.width; x++) {
            ...
            if (Math.random() < 0.05) type = TILE_TYPES.TREE;
            if (Math.random() < 0.08) type = TILE_TYPES.WATER;
            row.push({ type, x, y });
        }
        state.map.tiles.push(row);
    }
...
function initEntities() {
    state.entities = [
        { id: 1, name: 'Yarec', x: 50, y: 50, color: '#ffcc80', target: null, speed: 0.1, needs: { food: 100, rest: 100 }, path: [] },
        { id: 2, name: 'Arcenec', x: 52, y: 50, color: '#f48fb1', target: null, speed: 0.12, needs: { food: 100, rest: 100 }, path: [] },
        { id: 3, name: 'Timurec', x: 50, y: 52, color: '#90caf9', target: null, speed: 0.08, needs: { food: 100, rest: 100 }, path: [] }
    ];
}
...
function render() {
    ...
    for (let y = 0; y < state.map.height; y++) {
        for (let x = 0; x < state.map.width; x++) {
            const tile = state.map.tiles[y][x];
            const screenX = x * state.map.tileSize * zoom + offsetX;
            const screenY = y * state.map.tileSize * zoom + offsetY;
            ctx.fillStyle = tile.type.color;
            ctx.fillRect(screenX, screenY, state.map.tileSize * zoom, state.map.tileSize * zoom);
        }
    }
    
    for (const ent of state.entities) {
        const screenX = ent.x * state.map.tileSize * zoom + offsetX;
        const screenY = ent.y * state.map.tileSize * zoom + offsetY;
        ctx.fillStyle = ent.color;
        ctx.beginPath();
        ctx.arc(screenX + state.map.tileSize * zoom / 2, screenY + state.map.tileSize * zoom / 2, state.map.tileSize * zoom / 3, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = 'white';
        ctx.font = `${12 * zoom}px Arial`;
        ctx.fillText(ent.name, screenX + state.map.tileSize * zoom / 2, screenY - 5 * zoom);
    }
...
window.addEventListener('mousemove', (e) => {
    if (state.camera.isDragging) {
        state.camera.x += e.clientX - state.camera.lastMouseX;
        state.camera.y += e.clientY - state.camera.lastMouseY;
        state.camera.lastMouseX = e.clientX;
        state.camera.lastMouseY = e.clientY;
    }
});
